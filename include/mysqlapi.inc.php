...
function carousel()
{
    $resultat = requete("SELECT * FROM produit WHERE vendu = 0 ORDER BY date DESC LIMIT 5");
    $carou = "<div class=\"item active text-center\"><center><a href='ficheproduit.php?id=" . $resultat[0][0] . "' ><img class=\"img-rounded\" src = '" . $resultat[0][8] . "' height=\"600\" width=\"800\"></a></center><h2>" . $resultat[0][2] . "</h2></div>";
    for ($i = 1; $i <= 5 && isset($resultat[$i]); $i++) {
        $carou .= "\n\r<div class=\"item text-center\"><center><a href='ficheproduit.php?id=" . $resultat[$i][0] . "' ><img class=\"img-rounded\" src = '" . $resultat[$i][8] . "' height=\"600\" width=\"800\"></a></center><h2>" . $resultat[$i][2] . "</h2></div>";
    }
    return $carou;
}
